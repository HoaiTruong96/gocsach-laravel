        ...
        @foreach($badges->take($max) as $badge)
            ...
            <div class="group relative cursor-help" title="{{ $badge->name }}">
                @if($iconUrl)
                    <img src="{{ $iconUrl }}" 
                         alt="{{ $badge->name }}" 
                         class="{{ $sizeClass }} object-contain rounded-full"
                ...
                @elseif($icon && mb_strlen($icon) <= 4)
                    {{-- Emoji --}}
                    <span class="{{ $sizeClass }} inline-flex items-center justify-center bg-gradient-to-br from-purple-400 to-pink-400 rounded-full text-white">
                        {{ $icon }}
                    </span>
                @else
                    {{-- Fallback medal icon --}}
                    <span class="{{ $sizeClass }} inline-flex items-center justify-center bg-gradient-to-br from-yellow-400 to-orange-400 rounded-full text-white">
                        <i class="fas fa-medal" style="font-size: 60%;"></i>
                    </span>
                @endif
                
                {{-- Tooltip --}}
                <div class="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-1 hidden group-hover:block z-50 w-max pointer-events-none">
                    <div class="bg-gray-800 text-white text-[10px] rounded py-0.5 px-2 shadow-lg whitespace-nowrap">
                        {{ $badge->name }}
                    </div>
                    <div class="w-1.5 h-1.5 bg-gray-800 transform rotate-45 absolute -bottom-0.5 left-1/2 -translate-x-1/2"></div>
                </div>
            </div>
        @endforeach
